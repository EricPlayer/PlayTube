...
class TFSwipeShrinkView: UIView, UIGestureRecognizerDelegate {
    ...
    func initGestures() {
        ...
        let panGesture = UIPanGestureRecognizer(target: self, action: #selector(self.panning(panGesture:)))
        panGesture.minimumNumberOfTouches = 1
        panGesture.maximumNumberOfTouches = 1
        self.addGestureRecognizer(panGesture)
        
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(self.tapped(tapGesture:)))
